...
New first-class provider: **CA-AgentRouter** (`https://agentrouter.org`). Surfaces the AgentRouter gateway through Copilot Amplify with a single API key and a per-model transport — Claude Opus 4-x models go through the native Anthropic `messages` API; GPT, GLM, DeepSeek and other models go through the OpenAI-compatible chat-completions endpoint. The catalog is discovered dynamically from `GET {baseUrl}/api/pricing` and the picker is grouped under a single "CA-AgentRouter" vendor. **All six providers' externally-visible identifiers (vendor, display name, settings section, secret key) are now prefixed with `CA-` to avoid collisions with other extensions on a user's machine** — this affects every provider, not just the new one.

### Added

- **`CA-AgentRouter` provider (`LuneCode.CA-agentrouter`).** New `src/providers/agentrouter/` module with descriptor, dynamic catalog discovery, dual transport (Anthropic + OpenAI), thinking-effort support for Claude, and a managed `AmplifyProviderFacet` (testConnection, listModelsForTree, refreshCaches, health, warmupNow). Vendor `LuneCode.CA-agentrouter`, display name `CA-AgentRouter`, secret key `copilot-amplify.CA-agentrouter.apiKey`, settings section `copilot-amplify.CA-agentrouter.*`, tree icon `globe`, modelCountLabel `live`.
- **Native Anthropic transport (`src/core/api/anthropicClient.ts`).** New `AnthropicApiClient` speaking the Anthropic `messages` API (`POST /v1/messages`) with `x-api-key` + `anthropic-version: 2023-06-01` headers, SSE streaming (`message_start` / `content_block_start` / `content_block_delta` / `message_delta` / `message_stop`), `thinking: {type:'enabled', budget_tokens:N}` reasoning support, and `tool_use` content blocks mapped to VS Code `LanguageModelToolCallPart`. Lives in `core/api/` so future Anthropic-compatible gateways can reuse it.
- **Anthropic message converter (`src/core/context/anthropicConverter.ts`).** Converts VS Code `LanguageModelChatRequestMessage` → Anthropic `system` + `messages[]` + `tools[]`. System messages are concatenated into the top-level `system` string; user/assistant/tool-result parts are translated to the corresponding Anthropic content blocks; tools are mapped to `{name, description, input_schema}`.
- **Dynamic discovery via `GET {baseUrl}/api/pricing`.** Fetches the AgentRouter pricing endpoint, parses `{data: AgentrouterModel[]}` with `supported_endpoint_types: ['anthropic' | 'openai']` per model, applies a TTL cache (`agentrouter.cacheTtlSeconds`, default 300s), and yields the same `vscode.LanguageModelChatInformation[]` shape used by every other provider. Routing precedence: **(1) Claude is priority** — if the model id starts with `claude-` and `supported_endpoint_types` includes `anthropic`, the Anthropic transport wins even when both are advertised. **(2) Use the endpoint the server declares** — Claude with only `openai` advertised, non-Claude with only `anthropic` advertised, and any single-endpoint declaration all go to that single endpoint. **(3) Heuristic fallback** — when the server gives no `supported_endpoint_types` at all, `claude-*` goes to anthropic and everything else goes to openai.
- **Claude thinking-effort picker (`AGENTROUTER_CLAUDE_THINKING_CONFIGURATION`).** New per-family schema in `src/core/models/catalog.ts` exposing a binary `enabled` / `disabled` toggle for Claude reasoning. When enabled, the request body includes `thinking: {type:'enabled', budget_tokens:1024}` so the Anthropic transport surfaces thinking deltas as `LanguageModelThinkingPart` (or as a structured reasoning block when running on older VS Code versions).
- **Settings** (all under `copilot-amplify.CA-agentrouter.*`): `baseUrl` (default `https://agentrouter.org/v1`), `discoveryTimeoutMs` (default 8000), `requestTimeoutMs` (default 120000), `cacheTtlSeconds` (default 300), `warmupOnStartup` (default `false`).
- **Package metadata:** `package.json` `contributes.languageModelChatProviders[]` gains the `LuneCode.CA-agentrouter` entry (displayName `CA-AgentRouter`); `keywords[]` gains `"agentrouter"`. The same generic `copilot-amplify.manage`, `setApiKey`, `testConnection`, and `clearApiKey` commands used by every other provider drive the new entry — no new commands, no new menus.

### Changed

- **Every provider's external identifiers are now prefixed with `CA-`.** Vendors are now `LuneCode.CA-xiaomi` / `LuneCode.CA-glm` / `LuneCode.CA-groq` / `LuneCode.CA-nvidia` / `LuneCode.CA-omniroute` / `LuneCode.CA-agentrouter`; display names are `CA-Xiaomi MiMo` / `CA-Z.ai GLM` / `CA-Groq` / `CA-NVIDIA NIM` / `CA-Omniroute` / `CA-AgentRouter`. Secret keys are now `copilot-amplify.CA-<provider>.apiKey`. The OmniRoute settings section is now `copilot-amplify.CA-omniroute.*` (all 11 settings, including the base URL, timeouts, and toggles, are renamed) and the 10 OmniRoute-specific command IDs are now `copilot-amplify.CA-omniroute.*`. The other four fixed-catalog providers (Xiaomi, Z.ai, Groq, NVIDIA NIM) have no per-provider settings; their only renamed identifier is the secret key.
- **Provider IDs are unchanged.** The internal `ProviderId` union still uses `'xiaomi' | 'glm' | 'groq' | 'nvidia' | 'omniroute' | 'agentrouter'` — these are only used inside this extension's registry and are not externally visible. Renaming them would have churned the registry code for no real benefit.
- **Settings migration.** `src/secretsMigration.ts` now maps the old unprefixed plaintext settings (`copilot-amplify.xiaomi.apiKey`, `copilot-amplify.glm.apiKey`, `copilot-amplify.groq.apiKey`, `copilot-amplify.nvidia.apiKey`) to the new prefixed SecretStorage locations. Users who had plaintext keys in `settings.json` from very old versions will have them migrated to the new secret keys automatically on first activation of 2.2.0. Users who already had keys in the old unprefixed secret location (`copilot-amplify.<provider>.apiKey`) will need to re-enter their key once via *Manage Providers → Set API Key* — the new secret key is the only one this build reads.

### Notes

- **One API key, two transports.** A single `copilot-amplify.CA-agentrouter.apiKey` secret is used for both the Anthropic and OpenAI endpoints. The OpenAI-compatible base URL is `https://agentrouter.org/v1`; the Anthropic base URL is `https://agentrouter.org` (no `/v1` suffix), derived from the same setting by stripping the `/v1` path component.
- **Discovery endpoint.** `/api/pricing` is used in preference to `/v1/models` because the pricing endpoint advertises `supported_endpoint_types` per model, which is the exact routing metadata the dual-transport provider needs. Each entry includes a `model_name` (used as the picker id and family), `model_ratio` / `completion_ratio` (for future cost display), and `enable_groups` (informational).
- **Catalog is fully dynamic** — there is no `AGENTROUTER_MODELS` static array in `catalog.ts`. The provider surfaces whatever AgentRouter advertises. The Claude thinking-effort schema is the only catalog-shaped asset and is keyed off `claude-*` model ids.
- **No new settings, commands, menus, or transport plumbing beyond the new files.** The Anthropic transport is the only net-new wire protocol in this release; the OpenAI-compatible side reuses the existing `GenericApiClient`. Streaming, retry, circuit breaker, and diagnostics layers apply to the new provider exactly as they do to OmniRoute.
- **Tree view** automatically reflects the discovered model list. `listModelsForTree` reads from the live cache, so newly added AgentRouter models appear without updating the extension once the cache TTL expires (or via the existing *Refresh Providers & Models* command).
